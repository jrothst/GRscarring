...
** Date: 01/07/2019
**
** Read raw data decennial census (1980, 1990, 2000) 
** and ACS files (all availables files)from IPUMS 
** 
*****************************************************
clear
cap project, doinfo
if _rc==0 {
  ...
  local dofile "ipums_doubleweight"
  local doasproject=0
}
...
local IPUMS   "`rawdata'/IPUMS"
local scratch "`pdir'/scratch"

if `doasproject' == 1 {
  project, original("`IPUMS'/usa_00007.dat.gz")
  project, original("`IPUMS'/usa_00007.cbk.txt")
}


!zcat `IPUMS'/usa_00007.dat.gz > `IPUMS'/usa_00007.dat
...
!rm `IPUMS'/usa_00005.dat
save `scratch'/`dofile'.dta, replace 

!gzip -f `scratch'/`dofile'.dta

if `doasproject' == 1 {
  project, creates(`scratch'/`dofile'.dta.gz) 
}
